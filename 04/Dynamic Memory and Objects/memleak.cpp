...
{
  int sum = 0;
  for(int i=0; i < len; i++){ //should be < as starts from 0
    sum += data[i];
  }
  return sum;
}
...
char* test2_reverse(char* word)
{
  ...
  otherword[len] = '\0';
  //delete [] word; //should not delete word before receive value
  return otherword;
}
...
int main(int argc, char* argv[])
{
  ...
  if(test == 1){
    ...
    for(int i=0; i < len; i++){//until len same as line 11
      data[i] = rand()%100;
    }
    int sum = test1(data, len);
    cout << "Test 1 sum is " << sum << endl;

    delete [] data;//should delete data as we need to return memory to heap
  }
