...
bool HashTable<K,V,Prober,Hash,KEqual>::empty() const
{
	...
	for(it = table_.begin(); it != table_.end(); ++it){
		if((*it != nullptr) && !(*it)->deleted) return false;
	}
	return true;
}
...
{
	double curr_alpha = (double) cntElements/CAPACITIES[mIndex_];
	if(newAlpha <= curr_alpha) resize();
...
{
		//HASH_INDEX_T hashidx;
		tablesize_ = 0;
		cntElements = 0;
		mIndex_++;
    if(mIndex_ == 28) throw std::logic_error("CAPACITIES FULL"); //loading factor comparision
		table_.resize(CAPACITIES[mIndex_], nullptr);
		prober_.m_ = CAPACITIES[mIndex_];
    std::vector<HashItem*> newHashTable = table_;
		for(size_t i = 0; i < table_.size(); i++){
				table_[i] = nullptr;
		}
		for(size_t i = 0; i < newHashTable.size(); i++){
			if(newHashTable[i] != nullptr){
				if(!(newHashTable[i]->deleted)){
					HashItem* val = newHashTable[i];
					insert(val->item);
					delete val;
				}
				else{
