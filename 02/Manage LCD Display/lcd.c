...
void lcd_init(void)
{
    ...
    DDRD |= ((1 << PD4) | (1 << PD5) | (1 << PD6) |(1 << PD7));//Command/data bits 4 thorugh 7
    DDRB |= (1 << PB0); //Register select
    DDRB |= (1 << PB1); //Enable 
    DDRB &= ~(1 << PB2); //Backlight control(Bit 2 in the DDRB register should never be set to a one.)      
...
void lcd_writecommand(unsigned char cmd)
{
    ...
    lcd_writenibble(cmd); //writenibble sets the 4-upper bits to PORTD
    /* Call lcd_writenibble to send LOWER four bits of "cmd" argument */
    lcd_writenibble(cmd << 4);//bit shits to left 4 so 4-lower bits goes to upper
    /* Delay 2ms */
    _delay_ms(2);
}
...
void lcd_writedata(unsigned char dat)
{
    ...
    lcd_writenibble(dat); //writenibble sets the 4-upper bits to PORTD
    /* Call lcd_writenibble to send LOWER four bits of "dat" argument */
    lcd_writenibble(dat << 4); //bit shits to left 4 so 4-lower bits goes to upper
    /* Delay 2ms */
    _delay_ms(2);
}
...
void lcd_writenibble(unsigned char lcdbits)
{
    ...
    PORTB |= (1 << PB1); //Set Enable from 0 to 1;
    PORTB |= (1 << PB1); //Make E longer Task3
    PORTB &= ~(1 << PB1); //Clear Enable from 1 to 0; 0->1->0 transition
}
